...
export class FieldInquiryService {
     ...
     */
    async createInquiry(data, dealershipId) {
        // Check if enquiry already exists
        const existingInquiry = await this.repository.findByWhatsAppNumberAndDealership(data.whatsappNumber, dealershipId);
    ...
    async bulkUpload(data, dealershipId) {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                // Check if enquiry already exists
                const existingInquiry = await this.repository.findByWhatsAppNumberAndDealership(whatsappNumber, dealershipId);
                // Create field inquiry
                const enquiry = await this.repository.createInquiry({
                    firstName,
                    lastName,
