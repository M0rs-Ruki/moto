...
export default function VehicleModelsSettings() {
  ...
  useEffect(() => {
    ...
    const cachedCategories = getCachedData<any[]>("cache_settings_categories", 300000); // 5 minutes
    if (cachedCategories) {
  ...
  const handleCreateVariant = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!hasPermission("settingsVehicleModels")) {
      alert("You don't have permission to create variants");
      return;
    }
    setSaving(true);
    try {
        ...
        <CardHeader className="space-y-2">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="w-full">
              <CardTitle className="text-lg sm:text-xl">Categories & Models</CardTitle>
              ...
              <Dialog open={modelDialogOpen} onOpenChange={setModelDialogOpen}>
                ...
                <DialogContent className="max-w-md p-4 sm:p-6">
                  ...
                  <form onSubmit={handleCreateModel} className="space-y-4 mt-4">
                    <div className="space-y-2">
                      <Label htmlFor="model-category" className="text-sm">Category</Label>
                      <select
                        id="model-category"
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="model-year" className="text-sm">Year (optional)</Label>
                      <Input
                        id="model-year"
                        type="number"
                        value={newModel.year}
              ...
              <Dialog open={variantDialogOpen} onOpenChange={setVariantDialogOpen}>
                <DialogContent className="max-w-md p-4 sm:p-6">
                  ...
                  <form onSubmit={handleCreateVariant} className="space-y-4 mt-4">
                    <div className="space-y-2">
                      <Label htmlFor="variant-model" className="text-sm">Model</Label>
                      <select
                        id="variant-model"
                        className="w-full p-2 border rounded text-sm"
                        value={newVariant.modelId}
                        onChange={(e) => setNewVariant({ ...newVariant, modelId: e.target.value })}
                        required
                      >
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="variant-name" className="text-sm">Variant Name</Label>
                      <Input
                        id="variant-name"
                              ...
                              <div key={model.id} className="border rounded-lg p-3 space-y-2">
                                <div className="flex items-center justify-between">
                                  <div>
                                    ...
                                    {model.year && (
                                      <div className="text-muted-foreground text-xs">({model.year})</div>
                                    )}
                                  </div>
                                  <div className="flex gap-2">
                                    <Button
                                      ...
                                      variant="outline"
                                      onClick={() => {
                                        setNewVariant({ modelId: model.id, name: "" });
      ...
      <Dialog open={deleteCategoryDialogOpen} onOpenChange={setDeleteCategoryDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete this category? This will also delete all models in this
              category and cannot be undone.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={deleteModelDialogOpen} onOpenChange={setDeleteModelDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete the model "{modelToDelete?.name}"? This will also delete
              all related visitor interests and test drives, and cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <div className="flex flex-col sm:flex-row justify-end gap-2 mt-4">
            <Button
      ...
      <Dialog open={deleteVariantDialogOpen} onOpenChange={setDeleteVariantDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete the variant "{variantToDelete?.modelName}.
              {variantToDelete?.name}"? This will also delete all related visitor interests and test
