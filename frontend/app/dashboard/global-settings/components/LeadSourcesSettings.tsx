...
export default function LeadSourcesSettings() {
  ...
  useEffect(() => {
    ...
    const cachedLeadSources = getCachedData<any[]>("cache_settings_lead_sources", 300000); // 5 minutes
    if (cachedLeadSources) {
      ...
      try {
        const cacheEntry = JSON.parse(sessionStorage.getItem("cache_settings_lead_sources") || '{}');
  ...
  const handleCreateLeadSource = async (e: React.FormEvent) => {
    ...
    if (!hasPermission("settingsLeadSources")) {
      alert("You don't have permission to create lead sources");
      return;
    }
    setSaving(true);
    try {
  ...
  const handleDeleteLeadSource = async () => {
    ...
    setSaving(true);
    try {
      await apiClient.delete("/lead-sources", { data: { id: leadSourceToDelete } });
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader className="space-y-2">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            ...
            <Dialog open={leadSourceDialogOpen} onOpenChange={setLeadSourceDialogOpen}>
              ...
              <DialogContent className="max-w-md p-4 sm:p-6">
                ...
                <form onSubmit={handleCreateLeadSource} className="space-y-4 mt-4">
                  <div className="space-y-2">
                    <Label htmlFor="lead-source-name" className="text-sm">Lead Source Name</Label>
                    <Input
                      id="lead-source-name"
                      value={newLeadSource}
              ...
              {leadSources.map((source) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className="text-sm sm:text-base font-medium">{source.name}</span>
                    {source.isDefault && (
      ...
      <Dialog open={deleteLeadSourceDialogOpen} onOpenChange={setDeleteLeadSourceDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Delete Lead Source</DialogTitle>
            <DialogDescription>
              Are you sure you want to delete this lead source? This action cannot be undone. If this
              lead source is being used by any digital enquiries, it cannot be deleted.
            </DialogDescription>
          </DialogHeader>
