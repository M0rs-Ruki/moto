...
export default function SessionsPage() {
    ...
    if (cachedSessions && cachedCategories) {
      ...
      try {
        const cacheEntry = JSON.parse(
          sessionStorage.getItem("cache_sessions") || "{}",
        );
  ...
  const confirmExitSession = async () => {
    ...
    setSubmitting(true);
    try {
      await apiClient.post("/sessions/exit", {
      ...
      toast.error("Failed to exit session. Please try again.");
    } finally {
      setSubmitting(false);
                      ...
                      {!isExited && (
                        <div className="flex flex-col sm:flex-row gap-2 pt-2 border-t">
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleExitSession(session)}
                            disabled={submitting}
                            className="w-full sm:w-auto text-xs sm:text-sm"
                          >
                            {submitting ? (
