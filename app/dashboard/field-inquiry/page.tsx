...
export default function FieldInquiryPage() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("/api/field-inquiry");
      setEnquiries(response.data.enquiries);

      // Fetch phone lookups for all enquiries
      const lookups: Record<string, PhoneLookup> = {};
      await Promise.all(
        response.data.enquiries.map(async (enquiry: FieldInquiry) => {
          try {
  ...
  const handleCreateEnquiry = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post("/api/field-inquiry", {
        ...formData,
  ...
  const handleBulkUpload = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post("/api/field-inquiry/bulk", uploadFormData, {
        headers: {
